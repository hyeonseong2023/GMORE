        ...
        if (err) {
            ...
            if (rows.length > 0) {
                for (let i = 0; i < rows.length; i++) {
                    console.log("데이터길이",rows.length);
                    if (rows[i].image_url != null &&rows[i].image_url !="") {
                        console.log(rows[i].image_url)
                        let image_url = rows[i].image_url;
                        let readFile = fs.readFileSync('public/img/board/' + image_url + '.jpg'); //이미지 파일 읽기
                        let encode = Buffer.from(readFile).toString('base64'); //파일 인코딩
                        newBoard[i] = {
                            board_id: rows[i].board_id,
...
router.post('/delete', (req, res) => {
    ...
    conn.query(sql, [deleteData], (err, rows) => {
        if (err) {
            console.log(err)
        } else {
            if (rows.length > 0) {
